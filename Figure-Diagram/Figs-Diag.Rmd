...
```{r}
NMI_mat <- NMIs(nuclei %>% select(SC3, Monocle, Seurat)) %>%
  as.data.frame() %>%
  mutate(method1 = rownames(.)) %>%
  pivot_longer(-method1, names_to = "method2", values_to = "NMI") %>%
  ...
  distinct() %>%
  arrange(pair)
NMI <- ggplot(NMI_mat, aes(x = pair, y = 0)) +
  geom_tile(aes(fill = NMI), col = "black", size = 1) +
  scale_fill_gradientn(colours = brewer.pal(9, "Spectral"),
                         limits = c(0, 1)) +
  geom_text(aes(label = round(NMI, 2)), size = 8) +
  my_theme() +
  guides(fill = FALSE) +
...
As we can see, the NMI between the three methods is initially quite low.
...
```{r}
NMI_mat <- NMIs(df %>% select(SC3, Seurat, Monocle)) %>%
  as.data.frame() %>%
  mutate(method1 = rownames(.)) %>%
  pivot_longer(-method1, names_to = "method2", values_to = "NMI") %>%
  filter(method1 != method2) %>%
  ...
  distinct() %>%
  arrange(pair)
NMI_Final <- ggplot(NMI_mat, aes(x = pair, y = 0)) +
  geom_tile(aes(fill = NMI), col = "black", size = 1) +
  scale_fill_gradientn(colours = brewer.pal(9, "Spectral"),
                         limits = c(0, 1)) +
  geom_text(aes(label = round(NMI, 2)), size = 8) +
...
```{r}
NMI_Final <- plot_grid(NMI_Final, NULL, ncol = 1, 
                       rel_heights = c(1, 2)) 
p_b <- plot_grid(NULL, pOG, pMerg, NULL, NMI_Final, ncol = 5, 
                 rel_widths = c(.1, 1, 1, .2, 1), scale = .95)
rm(NMI_Final, pMerg, pOG)
```
...
p <- plot_grid(p_a, ggplot() + theme_void(), p_b, rel_heights = c(10, .5, 3),
  ...
  draw_line(x = c(.85, 0.93), y = c(.40, .36),
            arrow = arrow(angle = 45, length = unit(10, "pt"),
                          type = "closed")) +
  # NMI text
  draw_text(text = "Normalized Mutual Information\n(NMI)",
            x = .85, y = .26, size = 15) +
  NULL

# Main curved arrows
save_plot(filename = here("Figure-Diagram", "figures", "panel.png"),
