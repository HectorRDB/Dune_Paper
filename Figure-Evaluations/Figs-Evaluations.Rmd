...
p3 <- ggplot(df %>% select(scenario, Merge_method, AUARIC) %>%
  ...
  labs(x = "", y = "", fill = "Scaled\nAUC",
       title = paste0("Replicability", 
                      # "\n", "3 clustering methods × 3",
                      # "Theta × 2 pairs of datasets =\n18 comparisons",
                      "")) +
...
p4 <- ggplot(df %>% select(scenario, Merge_method, AUARIC) %>%
  ...
  # n", "3 clustering methods × 3", "Theta × 4 datasets =\n36 comparisons
  theme(axis.text.y = element_text(size = 15),
        axis.text.x = element_text(size = 6),
        title = element_text(size = 12)) +
  # scale_x_discrete(labels = labels) +
...
second_row <- plot_grid(plot_grid(p3, NULL, nrow = 2, rel_heights = c(1.2, .1)),
                        p4, nrow = 1, labels = c("c)", "d)"),
                        rel_widths = c(.8, 1))
