...
p1 <- ggplot(df, aes(x = nb_garb, y = 100 * AURC, group = nb_garb)) +
  geom_hline(yintercept = 100, color = "darkred", size = 1.5, alpha = .5) +
...
```{r}
p <- plot_grid(p1, p3, labels = c('a)', 'b)'), label_size = 15,
                     scale = .9)
ggsave(filename = here("Figure-Stability", "figures", "new_panel.png"),
       bg = "transparent", plot = p, width = 11, height = 4)
```