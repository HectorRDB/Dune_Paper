...
We can visualize the data using the labels from the original publication, and reducing the 10 dimensions of the latent space from __scvi__ to 2 using t-SNE [@tsne1, @tsne2, @tsne3].
...
Since the dataset has more than $5000$ cells, __SC3__ is automatically run in hybrid mode to lower runtime. However, the process is still quite slow. The code below is run in the default mode. If you want to run it faster, set `default=FALSE`
...
__Seurat__ seems to perform better than __SC3__ here but still seems to overpartition the data (with the default parameters). 
